...
bool symTblClass::setSym(const string symbl, const double newVal) {
    ...
    if(isBinded(symbl)){
        string setCommand = "set " + bindedSymTbl[symbl] + " " + to_string(newVal) + "\r\n\r\n";
...
double symTblClass::GetValueOfBinded(string bind) {
    ...
    else if(bind == "/controls/current-engine/throttle")
    {
        return symTblClass::engine_throttle.load(memory_order_seq_cst);
    }
    else if(bind == "/engines/engine/rpm")
    {
        return symTblClass::engine_rpm.load(memory_order_seq_cst);
    }
    else
    {
        string message = "get " + bind + " \r\n\r\n";
        string num = "";
        bool inside = false;
        int size;
        symTblClass::client.send_data(message);
        message = symTblClass::client.receive(size);
        for(auto c : message){
            if(c == '\'')
                inside = !inside;
            else if(inside)
